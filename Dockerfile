...
RUN set -eux; \
    ...
    if [ -n "${GHIDRA_VERSION}" ]; then \
    RELEASE_PAGE="https://github.com/${GHIDRA_OWNER}/${GHIDRA_REPO}/releases/tag/Ghidra_${GHIDRA_VERSION}_build"; \
    else \
    RELEASE_PAGE="https://github.com/${GHIDRA_OWNER}/${GHIDRA_REPO}/releases/latest"; \
    fi; \
    echo "GitHub API lookup failed, falling back to release page scrape: ${RELEASE_PAGE}"; \
