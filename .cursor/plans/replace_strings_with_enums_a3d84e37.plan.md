...
1. **Enum type**: Use `class ToolName(str, Enum)` so that `ToolName.OPEN.value == "open"` (wire format) and enum members are serialization-friendly. Member names: `OPEN`, `GET_FUNCTIONS`, `MANAGE_BOOKMARKS`, etc. (PascalCase from kebab).
...
- In [registry.py](src/agentdecompile_cli/registry.py) (or a new `agentdecompile_cli/enums.py` if you prefer to keep registry lean), define `ToolName(str, Enum)` with one member per canonical tool, `value` = kebab-case string (e.g. `OPEN = "open"`, `GET_FUNCTIONS = "get-functions"`, …). Generate member names from kebab by uppercasing and replacing `-` with `_`.
