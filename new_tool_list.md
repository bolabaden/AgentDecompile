...
**Important:** In GUI mode, the `programPath` parameter is **optional** for most tools. When `programPath` is not provided, AgentDecompile automatically uses the currently active program in the Ghidra Code Browser. This makes interactive analysis more convenient - you can simply call tools without specifying which program to use, and AgentDecompile will operate on the program you're currently viewing.
...
- `serverUsername` (string, optional): For shared projects: Username for Ghidra Server authentication. If not provided, will check AGENT_DECOMPILE_SERVER_USERNAME environment variable.
- `serverPassword` (string, optional): For shared projects: Password for Ghidra Server authentication. If not provided, will check AGENT_DECOMPILE_SERVER_PASSWORD environment variable.
- `serverHost` (string, optional): For shared projects: Ghidra Server hostname or IP address. If not provided, will check AGENT_DECOMPILE_SERVER_HOST environment variable.
- `serverPort` (integer, optional): For shared projects: Ghidra Server port (default: 13100). If not provided, will check AGENT_DECOMPILE_SERVER_PORT environment variable.
- `forceIgnoreLock` (boolean, optional): For projects: whether to forcibly ignore lock files by deleting them before opening (default: false, or AGENT_DECOMPILE_FORCE_IGNORE_LOCK env var)
...
### 30. `capture-agentdecompile-debug-info`

**Description:** Creates a zip file containing AgentDecompile debug information for troubleshooting issues. Includes system info, Ghidra config, AgentDecompile settings, MCP server status, open programs, and logs.
...
- **Project Management**: `open`, `list-project-files`, `list-open-programs`, `get-current-program`, `get-current-address` (GUI), `get-current-function` (GUI), `open-program-in-code-browser` (GUI), `checkin-program`, `analyze-program`, `change-processor`, `manage-files`, `capture-agentdecompile-debug-info`
