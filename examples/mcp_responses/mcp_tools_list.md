...
>
> Live local validation note: the current default `agentdecompile-server` advertisement is 37 tools. Hidden compatibility tools such as `manage-comments` still remain callable, and `switch-project` is still accepted as an alias to `open` even though it is not advertised.
