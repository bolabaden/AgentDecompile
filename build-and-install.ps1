# AgentDecompile Build and Install Script
...
function Ensure-RestartManagerTypeLoaded {
    if ("AgentDecompile.RestartManager" -as [type]) {
        return
    }
...
function Get-LockingProcessDetails {
    ...
    try {
        $procs = [AgentDecompile.RestartManager]::GetLockingProcesses($Path)
    } catch {
        return @()
    }
...
Write-Host "=== AgentDecompile Build and Install Script ===" -ForegroundColor Green
Write-Host ""
...
if (-not (Test-Path -Path $ProjectDir -PathType Container)) {
    ...
    $ProjectDir = Get-PathInput -Prompt "Enter project directory path" -Description "Enter the full path to the agentdecompile project directory"
}
...
try {
    $extensionFolderName = "agentdecompile"
