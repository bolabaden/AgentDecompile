...
LOG_FILE="/tmp/agentdecompile-install-ghidra.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

# Only run in remote (web) environments
if [ -n "${GHIDRA_INSTALL_DIR}" ]; then
    echo "GHIDRA_INSTALL_DIR is not set, use @agentdecompile-setup-installer to install the Ghidra binary distribution"
fi
