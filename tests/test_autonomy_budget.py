...
def test_frontdoor_exposes_autonomy_budget_flags() -> None:
    ...
    assert "autonomous_max_rewrite_requests" in dests
    ...
    ns = build_reconstruct_namespace(
        ...
        autonomous_max_rewrite_requests=2,
    ...
    assert ns.autonomous_max_rewrite_requests == 2
    assert ns.autonomous_max_wall_seconds == 120
    assert ns.autonomous_max_campaigns == 3
    assert ns.autonomous_stop_on_accept is True


def test_budget_accepts_rewrite_requests_field() -> None:
    budget = AutonomyBudget(max_rewrite_requests_per_function=3)
    assert budget.max_rewrite_requests_per_function == 3
    payload = budget.to_json()
    assert payload["max_rewrite_requests_per_function"] == 3


def test_budget_rejects_negative_rewrite_requests() -> None:
    with pytest.raises(ValueError):
        AutonomyBudget(max_rewrite_requests_per_function=-1)


def test_remaining_rewrite_requests() -> None:
    budget = AutonomyBudget(max_rewrite_requests_per_function=2)
    assert remaining_rewrite_requests(requests_seen=0, budget=budget) == 2
    assert remaining_rewrite_requests(requests_seen=2, budget=budget) == 0
