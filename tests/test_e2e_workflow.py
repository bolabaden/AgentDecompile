"""End-to-end workflow tests for AgentDecompile.
...
class TestE2EWorkflow:
    ...
    async def test_import_change_commit_reopen_workflow(
        self,
        ...
        import_result: dict[str, Any] = await mcp_stdio_client.call_tool(
            "import-file",
            arguments={
                "path": test_binary,
                "destinationFolder": "/",
        ...
        assert len(content) > 0, "Result content is empty"

        # Check if it's an error
        if hasattr(import_result, "isError") and import_result is not None and import_result.isError:
        ...
        decomp_result = await mcp_stdio_client.call_tool(
            "get-decompilation",
            arguments={"programPath": program_path, "functionNameOrAddress": "entry", "offset": 1, "limit": 10},
        )

        assert decomp_result is not None
        decomp_content = decomp_result.content
        decomp_data = json.loads(decomp_content[0].text)
        ...
        comment_result = await mcp_stdio_client.call_tool(
            "set-comment",
            arguments={"programPath": program_path, "addressOrSymbol": "entry", "comment": "E2E Test Comment - Auto-Checkout Verification"},
        )

        assert comment_result is not None
        comment_content = comment_result.content
        comment_data = json.loads(comment_content[0].text)
        ...
        checkin_result = await mcp_stdio_client.call_tool(
            "checkin-program",
            arguments={"programPath": program_path, "message": "E2E test: Added comment", "keepCheckedOut": False},
        )

        assert checkin_result is not None
        checkin_content = checkin_result.content
        checkin_data = json.loads(checkin_content[0].text)
        ...
        get_comments_result = await mcp_stdio_client.call_tool("get-comments", arguments={"programPath": program_path, "addressOrSymbol": "entry"})

        assert get_comments_result is not None
        comments_content = get_comments_result.content
        comments_data = json.loads(comments_content[0].text)
    ...
    async def test_auto_checkout_on_first_open(self, mcp_stdio_client: ClientSession, isolated_workspace: str):
        ...
        print("\n=== Import binary ===")
        import_result = await mcp_stdio_client.call_tool("import-file", arguments={"path": test_binary, "destinationFolder": "/", "enableVersionControl": False})
        ...
        label_result = await mcp_stdio_client.call_tool(
            "create-label",
            arguments={"programPath": program_path, "addressOrSymbol": "entry", "labelName": "auto_checkout_test_label"},
        )
    ...
    async def test_cache_release_during_checkin(self, mcp_stdio_client: ClientSession, isolated_workspace: str):
        ...
        assert len(content) > 0, "Result content is empty"
        if hasattr(import_result, "isError") and import_result.isError:
            content_0 = None if content is None else import_result.content[0]
            pytest.fail(f"Import failed: {'Unknown error' if content is None else content_0.text}")
        ...
        import_data: dict[str, list[str]] = json.loads(content_text)
        program_path: str = import_data["importedPrograms"][0]

        # Open the program by getting decompilation (this caches it)
        await mcp_stdio_client.call_tool("get-decompilation", arguments={"programPath": program_path, "functionNameOrAddress": "entry", "limit": 5})
        print("✓ Program opened and cached")

        print("\n=== Make changes ===")
        await mcp_stdio_client.call_tool("set-comment", arguments={"programPath": program_path, "addressOrSymbol": "entry", "comment": "Cache release test"})
        print("✓ Changes made")
        ...
        checkin_result: dict[str, list[str]] = await mcp_stdio_client.call_tool(
            "checkin-program", arguments={"programPath": program_path, "message": "Test cache release", "keepCheckedOut": True}
        )

        checkin_data: dict[str, list[str]] = json.loads(checkin_result.content[0].text)
        print(f"Checkin response: {json.dumps(checkin_data, indent=2)}")
