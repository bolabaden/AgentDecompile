...
AgentDecompile now supports authentication for shared Ghidra projects (projects connected to a Ghidra Server). This allows you to open remote projects that require username/password authentication.

## Overview

When opening a `.gpr` file that is connected to a Ghidra Server, authentication may be required. AgentDecompile supports providing credentials and server address information in two ways:

1. **Tool Parameters**: Pass `serverUsername`, `serverPassword`, `serverHost`, and `serverPort` directly to the `open` tool
2. **Environment Variables**: Set `AGENT_DECOMPILE_SERVER_USERNAME`, `AGENT_DECOMPILE_SERVER_PASSWORD`, `AGENT_DECOMPILE_SERVER_HOST`, and `AGENT_DECOMPILE_SERVER_PORT` environment variables
...
**Windows (PowerShell):**
```powershell
$env:AGENT_DECOMPILE_SERVER_USERNAME = "myuser"
$env:AGENT_DECOMPILE_SERVER_PASSWORD = "mypassword"
```

**Linux/Mac (Bash):**
```bash
export AGENT_DECOMPILE_SERVER_USERNAME="myuser"
export AGENT_DECOMPILE_SERVER_PASSWORD="mypassword"
```
...
```json
{
  "mcpServers": {
    "agentdecompile": {
      ...
      "args": [
        "--project",
        "G:/GitHub/Andastra/.cursor/mcp/agentdecompile",
        "run",
        "--module",
        "agentdecompile_cli"
      ],
      "env": {
        ...
        "AGENT_DECOMPILE_PROJECT_PATH": "G:/GitHub/Andastra/Odyssey.gpr",
        "AGENT_DECOMPILE_SERVER_USERNAME": "myuser",
        "AGENT_DECOMPILE_SERVER_PASSWORD": "mypassword",
...
**Note**: `AGENT_DECOMPILE_SERVER_HOST` and `AGENT_DECOMPILE_SERVER_PORT` are optional. The server address is typically stored in the project file and will be used automatically.
...
```json
{
  ...
  "arguments": {
    "path": "G:/Projects/Odyssey.gpr",
    "serverUsername": "override_user",  // This overrides AGENT_DECOMPILE_SERVER_USERNAME
    "serverPassword": "override_pass"   // This overrides AGENT_DECOMPILE_SERVER_PASSWORD
  }
}
```
...
1. **Credential Resolution**: AgentDecompile checks tool parameters first, then falls back to environment variables
2. **Authentication Setup**: If credentials are found, AgentDecompile sets up `PasswordClientAuthenticator` using `ClientUtil.setClientAuthenticator()` **before** opening the project
3. **Project Opening**: Ghidra's `GhidraProject.openProject()` uses the configured authenticator automatically
4. **Connection Check**: After opening, AgentDecompile checks if the project is shared and if the server connection succeeded
...
{
  "success": false,
  "error": "Shared project requires authentication but no credentials provided. Please provide 'serverUsername' and 'serverPassword' parameters, or set AGENT_DECOMPILE_SERVER_USERNAME and AGENT_DECOMPILE_SERVER_PASSWORD environment variables."
}
```
...
{
  "success": false,
  "error": "Authentication failed for shared project. Error: Invalid credentials. Please verify your username and password are correct. You can provide credentials via 'serverUsername'/'serverPassword' parameters or AGENT_DECOMPILE_SERVER_USERNAME/AGENT_DECOMPILE_SERVER_PASSWORD environment variables."
}
```
...
```json
{
  "mcpServers": {
    "agentdecompile": {
      "env": {
        "AGENT_DECOMPILE_SERVER_USERNAME": "analyst1",
        "AGENT_DECOMPILE_SERVER_PASSWORD": "secure_password"
      }
    }
  }
}
