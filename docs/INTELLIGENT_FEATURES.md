...
export AGENT_DECOMPILE_AUTO_BOOKMARK_PERCENTILE=98.0
```
...
Automatically applies function tags based on function analysis, API calls, library imports, and string patterns. When tags are not provided, AgentDecompile automatically analyzes the function and applies appropriate tags.
...
```json
// Auto-tag (controlled by AGENT_DECOMPILE_AUTO_TAG environment variable)
{
  "mode": "add",
  "function": "encrypt_data"
  // tags not provided - will be auto-tagged if AGENT_DECOMPILE_AUTO_TAG=true
}
...
export AGENT_DECOMPILE_AUTO_TAG=true
```
...
Enabled by default via `AGENT_DECOMPILE_AUTO_LABEL` environment variable in:
- `manage-function` (rename_function, rename_variable)
- `manage-symbols` (create_label, rename_data)

When names are not provided, AgentDecompile automatically assigns appropriate names based on context.
...
Enabled by default via `AGENT_DECOMPILE_AUTO_LABEL` environment variable in `manage-comments`.

When comment type or text is not provided, AgentDecompile automatically assigns appropriate values based on context.
...
export AGENT_DECOMPILE_AUTO_LABEL=true
...
1. **Let Auto-Labeling Help**: Keep `AGENT_DECOMPILE_AUTO_LABEL=true` (default) to get automatic names, then refine manually
...
AgentDecompile's intelligent features work together to:
