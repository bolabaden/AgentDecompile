...
**AgentDecompile Support:**
- AgentDecompile supports shared projects via authentication parameters
- See `docs/SHARED_PROJECT_AUTHENTICATION.md` for details

### Option 2: Workaround - Force Ignore Lock (RISKY)

You can set `AGENT_DECOMPILE_FORCE_IGNORE_LOCK=true` to delete lock files before opening:

```bash
# Windows PowerShell
$env:AGENT_DECOMPILE_FORCE_IGNORE_LOCK = "true"
mcp-agentdecompile

# Linux/Mac
export AGENT_DECOMPILE_FORCE_IGNORE_LOCK=true
mcp-agentdecompile
```
...
When `AGENT_DECOMPILE_FORCE_IGNORE_LOCK=true` is set, AgentDecompile attempts to delete lock files using:
...
- **AGENT_DECOMPILE_FORCE_IGNORE_LOCK** is a risky workaround
