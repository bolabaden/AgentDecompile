...
def build_validate_cases(program_path: str) -> list[dict[str, Any]]:
    ...
    return [
        {
            "name": "open",
            "uvx": f"{uvx_prefix} open --server_host 170.9.241.140 --server_port 13100 --server_username OpenKotOR --server_password MuchaShakaPaka {program_path}",
            "tool": "open",
            "args": {
        ...
        },
        {
            ...
            "args": {"program_path": program_path, "query": "SaveGame", "max_results": 20},
        },
        {
            "name": "references to",
            "uvx": f"{uvx_prefix} references to --binary {program_path} --target SaveGame --limit 25",
            "tool": "get_references",
            "args": {"program_path": program_path, "mode": "to", "target": "SaveGame", "limit": 25},
        },
    ]
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    cases = build_validate_cases(args.program_path)
...
def build_verify_cases(program_path: str) -> list[dict[str, Any]]:
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    cases = build_verify_cases(args.program_path)
    summary = []
    Path("tmp").mkdir(exist_ok=True)
