...
def roundtrip_one(binary: Path, timeout: int) -> dict:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="recovery-rt-", dir="/tmp"))
    try:
        src = tmp / "full-binary.S"
        obj = tmp / "full-binary.o"
        reb = tmp / "rebuilt.bin"
        src.write_text(
            '.section .recovery_image,"a"\n'
            ".global recovery_full_binary_start\n"
            "recovery_full_binary_start:\n"
            f"  .incbin {json.dumps(str(binary.resolve()))}\n"
            "recovery_full_binary_end:\n"
        )
        ...
        oc = subprocess.run(["objcopy", "-O", "binary", "-j", ".recovery_image", obj.name, reb.name],
                            cwd=tmp, capture_output=True, text=True, timeout=timeout)
...
def main() -> int:
    ...
    manifest = {
        "schema": "agentdecompile.roundtrip-batch.v1",
