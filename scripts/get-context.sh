#!/usr/bin/env bash
# Run global.getContextScript from agentdecompile.yaml (m2ctx / type headers).
#
# Usage:
...
script="$(agentdecompile_config_get global.getContextScript)" || {
  echo "get-context: global.getContextScript not set in agentdecompile config" >&2
  exit 1
}
...
if ! bash -c "$(printf '%s' "$script" | agentdecompile_expand_templates)" >"$output" 2>"$log"; then
