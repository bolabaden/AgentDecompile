#!/usr/bin/env bash
# Shared objdiff runner/parser for Recovery verification gates.
set -euo pipefail
...
verify_objdiff_main() {
  ...
  if [[ -z "$target" || -z "$candidate" ]]; then
    jq -n --arg message "target and candidate paths required" \
      '{schema:"agentdecompile.verify-objdiff.v1", status:"error", differences:-1, message:$message}'
    return 1
  fi
  if [[ ! -f "$target" ]]; then
    jq -n --arg file "$target" --arg message "Target file not found: $target" \
      '{schema:"agentdecompile.verify-objdiff.v1", status:"error", differences:-1, message:$message, file:$file}'
    return 1
  fi
  if [[ ! -f "$candidate" ]]; then
