...
matcher_parse_main() {
    ...
    if grep -qE '^[[:space:]]*(#include|__asm__[[:space:]]*\(|[A-Za-z_][A-Za-z0-9_[:space:]\*]*\([^;]*\)[[:space:]]*\{|typedef|struct|enum|static[[:space:]])' "$input"; then
      ...
      if [[ -n "$json_file" ]]; then
        ...
        jq -n --arg status "$status" --arg reason "$reason" --arg input "$input" \
          '{schema:"agentdecompile.matcher-parse.v1", status:$status, reason:$reason, input:$input, output:null, bytes:0}' >"$json_file"
      fi
  ...
  if [[ -n "$json_file" ]]; then
    ...
    jq -n \
      ...
      '{schema:"agentdecompile.matcher-parse.v1", status:"parsed", reason:null, input:$input, output:$output, bytes:$bytes}' >"$json_file"
  fi
}
