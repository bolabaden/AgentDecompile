#!/usr/bin/env python3
"""Run decomp-permuter for a Recovery prompt folder (Cursor-native bridge)."""
...
def main() -> int:
    ...
    config_path = Path(args.config) if args.config else project_root / "agentdecompile.yaml"
    if not config_path.is_file():
        config_path = project_root / "agentdecompile.example.yaml"
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="recovery-permuter-"))
    try:
