...
if [[ "$all_prompts" -eq 1 ]]; then
  if [[ ! -d "$prompts_dir" ]]; then
    jq -n \
      --arg schema "agentdecompile.decomp-readiness-summary.v1" \
...
if [[ ! -d "$prompt_dir" ]]; then
  jq -n \
    --arg schema "agentdecompile.decomp-readiness.v1" \
...
if [[ "$custom_verifier_configured" -eq 1 ]]; then
  ...
  if [[ -n "$compiler_command" ]]; then
    ...
    agentdecompile_cfg=""
    if agentdecompile_config_resolve >/dev/null 2>&1; then
      agentdecompile_cfg="$AGENTDECOMPILE_CONFIG_PATH"
      compiler_command="$(agentdecompile_config_get "global.compilerScript" optional || true)"
      [[ -n "$compiler_command" ]] && compiler_source="$agentdecompile_cfg"
    fi
  fi
