...
def build_aggregate_roundtrip(
    ...
    report = {
        "schema": "agentdecompile.elf-aggregate-source-roundtrip.v1",
        "binary": str(binary),
...
def auto_match(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="recovery-auto-trivial-") as tmp:
        tmp_dir = Path(tmp)
        for sym in symbols:
    ...
    report = {
        "schema": "agentdecompile.elf-auto-trivial.v1",
        "binary": str(binary),
        "status": "completed",
