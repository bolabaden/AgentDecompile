...
def feature_row(row: dict[str, Any], *, matched: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    output = {
        "schema": "agentdecompile.source-parity-function-features.v1",
        "name": row.get("name"),
        "entry": row.get("entry"),
...
def retrieve(remaining: list[dict[str, Any]], matched: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
    ...
    for item in remaining:
        ...
        rows.append(
            {
                "schema": "agentdecompile.source-parity-retrieval.v1",
                "name": item["name"],
                "entry": item["entry"],
...
def main() -> int:
    ...
    strategy = {
        "schema": "agentdecompile.source-parity-strategy-index.v1",
    ...
    summary = {
        "schema": "agentdecompile.source-parity-feature-index-summary.v1",
        "matchedExamples": len(matched_features),
        "remainingIndexed": len(remaining_features),
