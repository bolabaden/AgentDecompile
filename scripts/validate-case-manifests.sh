...
validate_verifier_report() {
  ...
  if ! jq -e --arg prompt "$prompt_name" '
    .schema == "agentdecompile.build-and-verify.v1"
    and .prompt == $prompt
...
validate_programmatic_phase_report() {
  ...
  if ! jq -e --arg prompt "$prompt_name" '
    .schema == "agentdecompile.programmatic-phase.v1"
    and .prompt == $prompt
...
validate_ai_phase_report() {
  ...
  if ! jq -e --arg prompt "$prompt_name" '
    .schema == "agentdecompile.ai-phase.v1"
    and .prompt == $prompt
...
validate_decomp_function_receipt() {
  ...
  if ! jq -e --arg prompt "$prompt_name" '
    .schema == "agentdecompile.decomp-function.v1"
    and .prompt == $prompt
    and (.exitCode | type == "number")
    ...
    integrated)
      ...
      if [[ -n "$receipt_path" ]]; then
        ...
        if [[ ! -f "$receipt_path" ]]; then
          ...
          .schema == "agentdecompile.integration-receipt.v1"
