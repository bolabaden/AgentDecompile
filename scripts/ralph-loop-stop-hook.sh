...
1. Advance PRIMARY one-shot coverage toward >=90% verified matched functions (objdiff 0 is the bar for claims).
2. Keep upstream Recovery bridges intact (vacuum, decomp-cli, source-parity-one-shot orchestrator, synthesis).
3. Bootstrap secondary PE target pipeline (discover→inventory→matching) until >=90% verified coverage.
