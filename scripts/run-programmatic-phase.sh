#!/usr/bin/env bash
# Orchestrate Recovery programmatic phase: get-context → m2c → compile/objdiff → permuter.
set -euo pipefail
...
Runs the one-way programmatic pipeline from the Macabeus/Recovery article:
...
write_report() {
  ...
  jq -n \
    --arg schema "agentdecompile.programmatic-phase.v1" \
    --arg status "$status" \
