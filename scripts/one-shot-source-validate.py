...
def validate_package_manifest(package: Path) -> list[str]:
    ...
    if manifest.get("schema") != "agentdecompile.one-shot-source-package-manifest.v1":
...
def validate_content_manifest(package: Path) -> list[str]:
    ...
    if manifest.get("schema") != "agentdecompile.one-shot-source-content-manifest.v1":
...
def validate_claims(package: Path) -> list[str]:
    ...
    if claims.get("schema") != "agentdecompile.one-shot-source-claims.v1":
...
def validate_authority_gates(package: Path) -> list[str]:
    ...
    if gates_doc.get("schema") != "agentdecompile.one-shot-source-authority-gates.v1":
...
def validate_authority_summary(
    ...
    if proof_commands.get("schema") != "agentdecompile.one-shot-source-proof-commands.v1":
    ...
    if deliverable_path.exists():
        ...
        if response_proof_path.exists():
            ...
            if response_proof.get("schema") != "agentdecompile.one-shot-source-byte-accurate-response-proof.v1":
        ...
        if bundle_verify_path.exists():
            ...
            if bundle_verify.get("schema") != "agentdecompile.one-shot-source-deliverable-verify.v1":
...
def validate_source_index(package: Path) -> list[str]:
    ...
    if index.get("schema") != "agentdecompile.one-shot-source-index.v1":
...
def validate_function_reconstruction_candidate_results(package: Path) -> list[str]:
    ...
    if results.get("schema") != "agentdecompile.one-shot-source-function-reconstruction-candidate-replay.v1":
...
def validate_one_shot_reconstruction_request(package: Path) -> list[str]:
    ...
    if not isinstance(request_json_proof, dict):
        ...
        if request_json.get("schema") != "agentdecompile.one-shot-source-reconstruction-request.v1":
        ...
        elif not isinstance(preferred.get("structuredShape"), dict) or preferred["structuredShape"].get("schema") != "agentdecompile.one-shot-source-reconstruction-response.v1":
    ...
    if not request_bundle_path.exists():
        ...
        if request_bundle.get("schema") != "agentdecompile.one-shot-source-reconstruction-request-bundle.v1":
...
def validate_one_shot_response_template(package: Path) -> list[str]:
    ...
    if not isinstance(shape, dict) or shape.get("schema") != "agentdecompile.one-shot-source-reconstruction-response.v1":
...
def validate_semantic_source_authority_evaluation(package: Path) -> list[str]:
    ...
    if evaluation.get("schema") != "agentdecompile.one-shot-source-semantic-authority-evaluation.v1":
...
def validate_package_proof(package: Path) -> list[str]:
    ...
    if proof.get("schema") != "agentdecompile.one-shot-source-package-proof.v1":
...
def validate_toolchain_provenance(package: Path) -> list[str]:
    ...
    if provenance.get("schema") != "agentdecompile.toolchain-provenance.v1":
...
def validate_verified_source_candidates(package: Path) -> list[str]:
    ...
    receipt = read_json(package / "one-shot-source-receipt.json")
    roles_doc = read_json(package / "SOURCE_ROLES.json")
    if manifest.get("schema") != "agentdecompile.verified-source-candidates.v1":
    ...
    if isinstance(supplied, dict):
        ...
        if recipe.get("schema") != "agentdecompile.candidate-build-recipe.v1":
        ...
        if report.get("schema") != "agentdecompile.supplied-source-candidate-roundtrip.v1":
...
def validate(
    ...
    return {
        "schema": "agentdecompile.one-shot-source-validate.v1",
        "package": str(package),
...
def validate_archive(path: Path, expect_content_identity: str | None = None, require_complete: bool = False) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="recovery-one-shot-validate-") as tmp:
        tmp_dir = Path(tmp)
