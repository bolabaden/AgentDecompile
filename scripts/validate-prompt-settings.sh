#!/usr/bin/env bash
# Validate Recovery prompt folder: settings.yaml has exactly functionName, targetObjectPath, asm.
set -euo pipefail
...
unless extra.empty?
  warn "unexpected keys (Recovery allows only 3): #{extra.sort.join(', ')}"
  exit 1
end
...
if extra:
    print(f"unexpected keys (Recovery allows only 3): {sorted(extra)}", file=sys.stderr)
    sys.exit(1)
if missing:
