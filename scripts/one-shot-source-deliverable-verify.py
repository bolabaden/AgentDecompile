...
def verify_deliverable(path: Path, timeout: int) -> dict[str, Any]:
    ...
    errors: list[str] = []
    if deliverable.get("schema") != "agentdecompile.one-shot-source-deliverable.v1":
        ...
        if response_proof_path is None:
            ...
            try:
                ...
                if (
                    ...
                    != "agentdecompile.one-shot-source-byte-accurate-response-proof.v1"
                ):
    ...
    ok = not errors
    return {
        "schema": "agentdecompile.one-shot-source-deliverable-verify.v1",
...
def verify_bundle(path: Path, timeout: int) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="recovery-one-shot-deliverable-bundle-") as tmp:
        tmp_dir = Path(tmp)
        ...
        if bundle_manifest is not None:
            if bundle_manifest.get("schema") != "agentdecompile.one-shot-source-deliverable-bundle-manifest.v1":
