...
- :func:`summarize_entries` – totals from an already-walked listing
...
GhidraKind = Literal["project", "repository"]
...
def _ghidra_kind(name: str) -> GhidraKind | None:
...
def summarize_entries(archive_path: Path, entries: Iterable[ArchiveEntry]) -> ArchiveSummary:
    """Build totals from an already-walked archive listing (one ZIP open)."""
    ...
    for entry in entries:
        kind = entry.ghidra_kind
        if entry.is_dir:
            dir_count += 1
            if kind == "project":
                ghidra_projects.add(entry.name.rstrip("/"))
            elif kind == "repository":
                ghidra_repos.add(entry.name.rstrip("/"))
            continue
        file_count += 1
...
def archive_summary(archive_path: Path) -> ArchiveSummary:
    ...
    return summarize_entries(archive_path, list_archive(archive_path))
