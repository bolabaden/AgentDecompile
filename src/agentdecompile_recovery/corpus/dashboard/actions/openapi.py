...
_KIND = {"str": str, "path": str, "int": int, "float": float, "bool": bool}
...
def model_for(action: ActionSpec) -> type[BaseModel]:
    ...
    for spec in action.fields:
        ...
        fields[safe] = (
            py | None,
            Field(default=default, alias=alias, description=spec.help or spec.name),
        )
...
def _params_from_body(action: ActionSpec, body: BaseModel) -> tuple[dict[str, Any], bool, bool, dict[str, Any]]:
    raw = body.model_dump(by_alias=True, exclude_none=True)
    confirm = bool(raw.pop("confirm", False))
    dry = bool(raw.pop("dryRun", False) or raw.pop("dry_run", False))
    context = raw.pop("context", None)
    if not isinstance(context, dict):
        context = {}
    allowed = {spec.name for spec in action.fields}
    params = {key: value for key, value in raw.items() if key in allowed}
    return params, confirm, dry, context
...
def _mount_action(router: APIRouter, action: ActionSpec) -> None:
    ...
    async def handler(request: Request) -> JSONResponse:
        ...
        try:
            body = model_cls.model_validate(raw)
        except Exception as exc:
            return JSONResponse({"error": str(exc)}, status_code=422)
        params, confirm, dry, context = _params_from_body(action, body)
        payload, status = start_job(
            action.id, params, context=context, confirm=confirm, dry_run=dry,
        )
