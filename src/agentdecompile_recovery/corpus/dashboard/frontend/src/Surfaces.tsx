...
export function RecoveryWorkspace({props,catalog=[],liveRun}:{props:SurfaceProps;catalog?:Data[];liveRun?:Data}) {
  ...
  return <Section id="recovery" title="Recovery and evidence">
    ...
    <dl className="facts"><div><dt>Recorded assembly-free source</dt><dd>{data.data.recovery?.sourceAvailability?.count!=null?`${data.data.recovery.sourceAvailability.count} functions${data.data.recovery.sourceAvailability.total!=null?` of ${data.data.recovery.sourceAvailability.total}`:''} · source witness, not proof`:"Source availability not measured for this scope"}{data.data.recovery?.sourceAvailability?.reason&&<><br/><small>{data.data.recovery.sourceAvailability.reason}</small></>}</dd></div><div><dt>Compilation</dt><dd>{data.data.recovery?.compilation?.summary||data.data.recovery?.compilation?.reason||'No scoped compilation receipt supplied'}</dd></div><div><dt>Byte verification</dt><dd>{data.data.recovery?.verification?.summary||data.data.recovery?.verification?.reason||'No scoped byte-verification receipt supplied'}</dd></div></dl>
