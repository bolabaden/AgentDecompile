...
# Names matching this pattern were either generated by Ghidra's auto-analysis or
# are autogen-shaped names that were written into the database by a script or
# importer. Either way they carry no curated human knowledge.
AUTOGEN_RE = re.compile(r"^(?:thunk_)?(?:FUN|SUB|DAT|LAB|UNK|EXT|switchD|caseD)_")
...
class Symbol:
    ...
    @property
    def has_non_autogen_name(self) -> bool:
        """Strict check: stored name that does not match known autogen prefixes.

        ``has_curated_name`` trusts Ghidra's empty-name convention. This
        property additionally rejects names whose shape matches ``AUTOGEN_RE``
        — i.e. names that were written into the database by a script or
        importer but still look like ``FUN_00401000`` or ``DAT_00601000``.
        """

        return bool(self.name) and not AUTOGEN_RE.match(self.name)
...
class Function:
    ...
    def has_non_autogen_name(self) -> bool:
        ...
        See ``Symbol.has_non_autogen_name`` for the rationale.
        """
...
class GhidraProgram:
    ...
    def names_by_entry(self, *, curated_only: bool = True) -> dict[int, str]:
        ...
        for function in self.functions():
            if curated_only and not function.has_curated_name:
                continue
...
def census_names(program: GhidraProgram) -> dict[str, Any]:
    """Return a name-quality census for *program*.

    Produces three counts each for functions and symbols:

    * ``total``          — every row in the respective table.
    * ``hasStoredName``  — rows that store a non-empty name (``has_curated_name``).
    * ``nonAutogen``     — rows whose stored name does not match ``AUTOGEN_RE``
                           (``has_non_autogen_name``).

    The delta between ``hasStoredName`` and ``nonAutogen`` exposes databases where
    autogen-shaped names were written in by a script or importer and would
    inflate a naïve curated-name count.
    """
    ...
    for fn in program.functions():
        func_total += 1
        if fn.has_curated_name:
            func_stored += 1
        if fn.has_non_autogen_name:
            func_strict += 1
    ...
    for sym in program.symbols():
        sym_total += 1
        if sym.has_curated_name:
            sym_stored += 1
        if sym.has_non_autogen_name:
            sym_strict += 1

    pct = round(100.0 * func_strict / func_total, 1) if func_total else 0.0

    return {
        "schema": "agentdecompile.name-census.v1",
        "program": program.name,
        "language": program.language_id,
        "functions": {
            "total": func_total,
            "hasStoredName": func_stored,
            "nonAutogen": func_strict,
            "curatedPercent": pct,
        },
        "symbols": {
            "total": sym_total,
            "hasStoredName": sym_stored,
            "nonAutogen": sym_strict,
        },
    }
