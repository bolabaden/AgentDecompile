...
This file provides guidance for Claude Code when working with the AgentDecompile MCP server architecture in the `agentdecompile.server` package.
...
The `agentdecompile.server` package contains the core MCP (Model Context Protocol) server implementation that enables AI-assisted reverse engineering through Ghidra. The server uses Jetty with streamable HTTP transport to provide real-time communication between AI clients and Ghidra's analysis capabilities.
...
┌─────────────────────────────────────────────────────────────────┐
│                    Ghidra Programs                              │
│              (via AgentDecompileProgramManager)                           │
└─────────────────────────────────────────────────────────────────┘
```
...
- **Service Registry**: Integration with AgentDecompileInternalServiceRegistry for component coordination
...
The central orchestrator for the entire MCP server infrastructure. Implements `AgentDecompileMcpService` interface.
...
threadPool = GThreadPool.getPrivateThreadPool("AgentDecompile");
AgentDecompileInternalServiceRegistry.registerService(GThreadPool.class, threadPool);
...
AgentDecompileInternalServiceRegistry.registerService(ConfigManager.class, configManager);
AgentDecompileInternalServiceRegistry.registerService(McpSyncServer.class, server);
AgentDecompileInternalServiceRegistry.registerService(McpServerManager.class, this);
AgentDecompileInternalServiceRegistry.registerService(AgentDecompileMcpService.class, this);
...
| Symptom | Cause | Solution |
|---------|-------|----------|
| Connection refused | Server not running | Ensure Ghidra is running with AgentDecompile enabled |
...
- `/src/main/java/agentdecompile/services/CLAUDE.md` - AgentDecompileMcpService interface
- `/src/main/java/agentdecompile/headless/CLAUDE.md` - Headless mode server usage
