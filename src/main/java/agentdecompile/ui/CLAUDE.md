...
This file provides guidance for Claude Code when working with the AgentDecompile user interface components in the `agentdecompile.ui` package.
...
| **Files** | AgentDecompileProvider.java, CaptureDebugAction.java |
| **Pattern** | Ghidra ComponentProvider/DockingAction |
| **Testing** | Requires `java.awt.headless=false` |

**Note**: This package is **lower priority** compared to core server/tools functionality. The primary AgentDecompile capabilities are exposed through the MCP server interface, making the UI optional for most use cases.

## Package Components

| Component | Purpose | Status |
|-----------|---------|--------|
| `AgentDecompileProvider` | Dockable status panel for server monitoring | Basic (TODO items) |
| `CaptureDebugAction` | Menu action for debug info capture | Complete |

## AgentDecompileProvider
...
```java
// Creation (currently commented out in AgentDecompileApplicationPlugin)
provider = new AgentDecompileProvider(plugin, getName());
...
- `agentdecompile.debug.DebugCaptureService` - Performs actual capture
...
action.setMenuBarData(new MenuData(new String[] { "Tools", "AgentDecompile", "Action" }));
action.setDescription("Action description");
action.setHelpLocation(new HelpLocation("AgentDecompile", "Topic"));
...
public void setUp() {
    ...
    plugin = new AgentDecompilePlugin(mockTool);
    provider = new AgentDecompileProvider(plugin, "Test");
}
```
...
| File | Description |
|------|-------------|
| `/src/main/java/agentdecompile/plugin/CLAUDE.md` | Plugin architecture, ConfigManager |
| `/src/main/java/agentdecompile/server/CLAUDE.md` | Server integration, status events |
| `/src/main/java/agentdecompile/debug/CLAUDE.md` | DebugCaptureService used by CaptureDebugAction |
