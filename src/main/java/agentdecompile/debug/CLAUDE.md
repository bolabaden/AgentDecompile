...
This package provides debug capture functionality for troubleshooting AgentDecompile issues.

## Overview

The debug capture feature creates a zip file containing diagnostic information that can be shared when reporting issues. It captures system info, Ghidra config, AgentDecompile status, open programs, and application logs.
...
- `collectAgentDecompileInfo()` - AgentDecompile configuration settings
...
**Menu**: Tools -> AgentDecompile -> Capture Debug Info
...
```
agentdecompile-debug-YYYY-MM-DDTHH-mm-ss.zip
