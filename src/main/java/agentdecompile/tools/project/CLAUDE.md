...
This file provides guidance to Claude Code (claude.ai/code) when working with the project tools package in AgentDecompile.

## Package Overview

The `agentdecompile.tools.project` package provides MCP tools for Ghidra project management operations. It handles program discovery, project file listing, and version control operations through Ghidra's project framework and version control system.
...
### Program Discovery Through AgentDecompileProgramManager

**Use AgentDecompileProgramManager for consistent program access across tools**:
```java
...
// AgentDecompileProgramManager handles:
...
// Then open it using AgentDecompileProgramManager.getProgramByPath()
```
...
- Both operations cache programs for future access via AgentDecompileProgramManager
...
- **AgentDecompileProgramManager fallback** - Tests may use direct program registration
...
- **AgentDecompileProgramManager integration**: Use AgentDecompileProgramManager.getOpenPrograms() for program discovery
