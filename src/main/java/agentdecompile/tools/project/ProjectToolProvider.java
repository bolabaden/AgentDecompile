 ...
 * <p>
 * Ghidra API: {@link ghidra.framework.model.Project}, {@link ghidra.framework.model.DomainFile},
 * {@link ghidra.framework.model.DomainFolder}, {@link ghidra.app.services.ProgramManager} -
 * <a href="https://ghidra.re/ghidra_docs/api/ghidra/framework/model/Project.html">Project API</a>,
 * <a href="https://ghidra.re/ghidra_docs/api/ghidra/framework/model/DomainFile.html">DomainFile API</a>.
...
public class ProjectToolProvider extends AbstractToolProvider {
    ...
    private void registerGetCurrentProgramTool() {
        ...
        registerTool(tool, (exchange, request) -> {
            ...
            // Ghidra API: Program.getDomainFile(), DomainFile.getPathname(), getLastModifiedTime(), isReadOnly() - https://ghidra.re/ghidra_docs/api/ghidra/framework/model/DomainObject.html#getDomainFile(), https://ghidra.re/ghidra_docs/api/ghidra/framework/model/DomainFile.html#getPathname()
    ...
    private void registerListOpenProgramsTool() {
        ...
        registerTool(tool, (exchange, request) -> {
            ...
            for (DomainFile domainFile : AgentDecompileProgramManager.getAllProgramFiles()) {
                try {
                    Map<String, Object> programInfo = new HashMap<>();
            ...
            try {
                ...
                if (domainFile.canAddToRepository()) {
                    ...
                    DefaultCheckinHandler checkinHandler = new DefaultCheckinHandler(
                        message + "\n💜🐉✨ (AgentDecompile)", keepCheckedOut, false);
                ...
                if (file.canAddToRepository()) {
                    try {
                        ...
                        String commitMessage = wasAnalyzed
                            ? "Initial import via AgentDecompile (analyzed)"
                            : "Initial import via AgentDecompile";
    ...
    private McpSchema.CallToolResult handleImportOperation(io.modelcontextprotocol.server.McpSyncServerExchange exchange, io.modelcontextprotocol.spec.McpSchema.CallToolRequest request) {
        ...
        if (importMonitor.isCancelled() && importedDomainFiles.isEmpty()) {
            return createErrorResult("Import timed out after " + importTimeoutSeconds + " seconds. "
                    + "Try importing fewer files or increase timeout in AgentDecompile configuration.");
        }
                        ...
                        try {
                            ...
                            if (domainObject instanceof Program program) {
                                ...
                                if (analysisManager != null) {
                                    ...
                                    if (!analysisMonitor.isCancelled()) {
                                        program.save("Analysis completed via AgentDecompile import", postMonitor);
    ...
    private McpSchema.CallToolResult handleUncheckoutOperation(io.modelcontextprotocol.spec.McpSchema.CallToolRequest request) {
        ...
        try {
            ...
            if (program != null) {
                AgentDecompileProgramManager.releaseProgramFromCache(program);
            }
        } catch (Exception e) {
    ...
    private void registerOpenTool() {
        ...
        properties.put("serverUsername", SchemaUtil.stringProperty(
                ...
                + "If not provided, will check AGENT_DECOMPILE_SERVER_USERNAME environment variable. "
        ...
        properties.put("serverPassword", SchemaUtil.stringProperty(
                ...
                + "If not provided, will check AGENT_DECOMPILE_SERVER_PASSWORD environment variable. "
                + "Required for shared projects connected to a Ghidra Server."
        ));
        properties.put("serverHost", SchemaUtil.stringProperty(
                "For shared projects: Ghidra Server hostname or IP address. "
                + "If not provided, will check AGENT_DECOMPILE_SERVER_HOST environment variable. "
        ...
        properties.put("serverPort", Map.of(
                ...
                + "If not provided, will check AGENT_DECOMPILE_SERVER_PORT environment variable. "
        ...
        properties.put("forceIgnoreLock", SchemaUtil.booleanPropertyWithDefault(
                "For projects: whether to forcibly ignore lock files by deleting them before opening (default: false, or AGENT_DECOMPILE_FORCE_IGNORE_LOCK env var). "
    ...
    private String[] getServerCredentials(Map<String, Object> request) {
        ...
        if (username == null || username.trim().isEmpty()) {
            username = System.getenv("AGENT_DECOMPILE_SERVER_USERNAME");
        }
        if (password == null || password.trim().isEmpty()) {
            password = System.getenv("AGENT_DECOMPILE_SERVER_PASSWORD");
        }
    ...
    private Object[] getServerAddress(Map<String, Object> request) {
        ...
        if (host == null || host.trim().isEmpty()) {
            host = System.getenv("AGENT_DECOMPILE_SERVER_HOST");
        }
        if (port == null) {
            String portStr = System.getenv("AGENT_DECOMPILE_SERVER_PORT");
        ...
        try {
            ...
            try {
                ...
                if (errorMsg != null && (errorMsg.contains("authentication")
                    ...
                    return createErrorResult(
                            ...
                            + "or AGENT_DECOMPILE_SERVER_USERNAME/AGENT_DECOMPILE_SERVER_PASSWORD environment variables."
                    );
                }
            ...
            if (shouldOpenAllPrograms) {
                for (DomainFile domainFile : allPrograms) {
                    ...
                    try {
                        // Use AgentDecompileProgramManager to open the program - this caches it for future access
    ...
    protected McpSchema.CallToolResult handleOpenProgram(Map<String, Object> request, String path) {
        try {
            ...
            if (existingProgram != null) {
                ...
                try {
                    existingProgram = importSingleFile(file, destFolder, analyzeAfterImport, enableVersionControl,
                            "Initial import via AgentDecompile open");
                    programPath = existingProgram.getPathname();
                    wasImported = true;
                } catch (Exception e) {
            ...
            // Open the program in memory using AgentDecompileProgramManager (this caches it)
            Program program = AgentDecompileProgramManager.getProgramByPath(programPath);
            if (program == null || program.isClosed()) {
    ...
    private void registerOpenProgramInCodeBrowserTool() {
        ...
        registerTool(tool, (exchange, request) -> {
            ...
            try {
                ...
                // If no Code Browser found, try to use AgentDecompilePlugin's tool if it has ProgramManager
                if (codeBrowserTool == null) {
                    agentdecompile.plugin.AgentDecompilePlugin agentdecompilePlugin = agentdecompile.util.AgentDecompileInternalServiceRegistry.getService(agentdecompile.plugin.AgentDecompilePlugin.class);
                    if (agentdecompilePlugin != null && agentdecompilePlugin.getTool() != null) {
                        PluginTool agentdecompileTool = agentdecompilePlugin.getTool();
                        ProgramManager testManager = agentdecompileTool.getService(ProgramManager.class);
                        if (testManager != null) {
                            codeBrowserTool = agentdecompileTool;
                            Msg.debug(this, "Using AgentDecompilePlugin's tool for opening program");
                        }
                    }
                }
        ...
        try {
            ...
            if (codeBrowserTool == null) {
                ...
                if (agentdecompilePlugin != null && agentdecompilePlugin.getTool() != null) {
                    ...
                    if (testManager != null) {
                        codeBrowserTool = agentdecompileTool;
                        Msg.debug(this, "Using AgentDecompilePlugin's tool for opening programs");
                    }
                }
            }
     ...
     * (e.g., "Initial import via AgentDecompile open")
    ...
    private McpSchema.CallToolResult handleExportFunctionInfo(Program program, io.modelcontextprotocol.spec.McpSchema.CallToolRequest request, String outputPath) {
        try {
            ...
            while (funcIter.hasNext()) {
                ...
                funcInfo.put("address", agentdecompile.util.AddressUtil.formatAddress(func.getEntryPoint()));
            ...
            while (dataIter.hasNext()) {
                ...
                if (data.hasStringValue()) {
                    ...
                    if (str != null && str.length() > 0) {
                        strings.add(agentdecompile.util.AddressUtil.formatAddress(data.getAddress()) + ": " + str);
                    }
                }
            }
