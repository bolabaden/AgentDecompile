...
This file provides guidance for working with the structures tools package in AgentDecompile (Reverse Engineering Assistant).

## Package Overview

The `agentdecompile.tools.structures` package provides comprehensive tools for creating, manipulating, and analyzing data structures in Ghidra programs through the MCP server. It enables AI-assisted reverse engineering by providing structured access to Ghidra's data type management capabilities.
