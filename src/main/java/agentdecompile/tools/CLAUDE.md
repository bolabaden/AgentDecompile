...
This file provides guidance to Claude Code (claude.ai/code) when working with the tools package in AgentDecompile.

## Package Overview

The `agentdecompile.tools` package implements MCP (Model Context Protocol) tool providers that expose Ghidra's reverse engineering capabilities to AI models. The package contains 17 specialized tool providers organized into five categories: Core Analysis, Data & Types, Advanced Analysis, Annotations, and Project Management. Each tool provider focuses on a specific domain and follows consistent patterns for parameter handling, error management, and JSON response formatting.
...
3. **Follow naming conventions**:
   ...
   - Package structure: `agentdecompile.tools.[domain]/[Domain]ToolProvider.java`
...
**ALWAYS use AgentDecompile utilities instead of direct Ghidra APIs** for consistency:
...
- `AgentDecompileInternalServiceRegistry.getService(ServiceClass.class)` - Access services like ConfigManager
...
- **Service access**: Use AgentDecompileInternalServiceRegistry to get services like ConfigManager
