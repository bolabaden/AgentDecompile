...
public class DataTypeToolProvider extends AbstractToolProvider {
    ...
    private McpSchema.CallToolResult handleApplyAction(io.modelcontextprotocol.spec.McpSchema.CallToolRequest request) {
        ...
        ghidra.program.model.address.Address address = agentdecompile.util.AddressUtil.resolveAddressOrSymbol(program, addressOrSymbol);
        if (address == null) {
            return createErrorResult("Could not resolve address or symbol: " + addressOrSymbol);
        }

        try {
            ghidra.program.model.data.DataType dataType = agentdecompile.util.DataTypeParserUtil.parseDataTypeObjectFromString(dataTypeString, archiveName);
            if (dataType == null) {
            ...
            try {
                ...
                resultData.put("address", agentdecompile.util.AddressUtil.formatAddress(address));
    ...
    private McpSchema.CallToolResult handleBatchApplyDataType(Program program, CallToolRequest request,
        ...
        try {
            dataType = agentdecompile.util.DataTypeParserUtil.parseDataTypeObjectFromString(dataTypeString, archiveName);
            if (dataType == null) {
                return createErrorResult("Could not find data type: " + dataTypeString);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            for (int i = 0; i < addressList.size(); i++) {
                try {
                    ...
                    Map<String, Object> result = new HashMap<>();
                    result.put("index", i);
                    result.put("address", agentdecompile.util.AddressUtil.formatAddress(address));
