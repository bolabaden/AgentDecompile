...
This file provides guidance to Claude Code when working with the constants tool provider in AgentDecompile.

## Package Overview

The `agentdecompile.tools.constants` package provides MCP tools for searching and analyzing constant values (immediate operands) used in program instructions. This helps identify magic numbers, error codes, buffer sizes, and other significant values across the codebase.
