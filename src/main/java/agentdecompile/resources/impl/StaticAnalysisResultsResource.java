...
public class StaticAnalysisResultsResource extends AbstractResourceProvider {
    ...
    private static final String RESOURCE_ID = "ghidra://static-analysis-results";
    private static final String RESOURCE_NAME = "static-analysis-results";
    private static final String RESOURCE_DESCRIPTION = "SARIF 2.1.0 format static analysis results for all open programs - comprehensive analysis including functions, symbols, comments, and findings";
    private static final String RESOURCE_MIME_TYPE = "application/sarif+json";
    ...
    public StaticAnalysisResultsResource(McpSyncServer server) {
        super(server);
    }
                ...
                (exchange, request) -> {
                    ...
                    try {
                        ...
                        for (Program program : openPrograms) {
                            ...
                            cachedDoc = cachedDoc == null || cachedDoc.isEmpty() ? "" : cachedDoc;
                            logInfo("Cached doc length: " + cachedDoc.length());
                            if (cachedDoc.isEmpty()) {
                                ...
                                future = future == null ? generateDocumentationAsync(program) : future;
                                try {
                        ...
                        for (Program program : openPrograms) {
                            ...
                            String cachedDoc = programDocumentationCache.get(program);
                            logInfo("Cached doc for final build - exists: " + (cachedDoc != null) + ", length: " + (cachedDoc.length()));
    ...
    /**
     * Pre-generate documentation for all currently open programs. Called during
     * resource registration to ensure documentation is ready immediately.
     */
    ...
    /**
     * Generates comprehensive JSON documentation for a single program. Collects
     * ALL data: functions, comments, symbols, bookmarks, strings, data types,
     * etc.
     */
    ...
    /**
     * Collect comprehensive function data including parameters, variables,
     * tags, and comments.
     */
    ...
    private void collectAllStrings(Program program, ArrayNode strings) {
        ...
        for (Data data : dataIterator) {
            if (data.getValue() instanceof String stringValue) {
    ...
    /**
     * Generate documentation for a program asynchronously. Returns a
     * CompletableFuture that completes when documentation is ready. If
     * generation is already in progress, returns the existing future.
     */
    private CompletableFuture<String> generateDocumentationAsync(Program program) {
        ...
        GThreadPool threadPool = AgentDecompileInternalServiceRegistry.getService(GThreadPool.class);
