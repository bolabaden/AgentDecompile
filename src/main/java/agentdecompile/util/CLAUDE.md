# CLAUDE.md - AgentDecompile Util Package

This file provides guidance for working with the foundational utility classes in the `agentdecompile.util` package. These utilities are used throughout ALL tool providers and form the backbone of AgentDecompile's consistent behavior patterns.

## Package Overview

The `agentdecompile.util` package contains 11 critical utility classes that provide:
...
- **AgentDecompileInternalServiceRegistry** - Service registry for dependency injection (ConfigManager, etc.)
...
**CRITICAL**: All tool providers MUST use these utilities instead of direct Ghidra API calls to ensure consistency across the entire AgentDecompile ecosystem.
...
| **AgentDecompileInternalServiceRegistry** | `registerService()`, `getService()`, `unregisterService()` | Service dependency injection |
...
### AgentDecompileInternalServiceRegistry - Service Dependency Injection

**PRIMARY RULE**: Use this registry to access core AgentDecompile services without tight coupling.
...
AgentDecompileInternalServiceRegistry.registerService(MyService.class, myServiceImpl);

// Unregister when shutting down
AgentDecompileInternalServiceRegistry.unregisterService(MyService.class);
```

#### Available Services:
- `ConfigManager.class` - AgentDecompile configuration management
- `AgentDecompilePlugin.class` - Main plugin instance
...
public void setUp() {
    ...
    AgentDecompileInternalServiceRegistry.registerService(ConfigManager.class, mockConfig);
}
...
- **NEVER bypass these utilities** - They ensure consistency across all AgentDecompile tools
- **All tools MUST use `AddressUtil.formatAddress()`** for address output in JSON
  - This ensures the consistent "0x" prefix format across all AgentDecompile responses
...
- **Service registry provides loose coupling** - use it for accessing AgentDecompile services (ConfigManager, AgentDecompilePlugin)
