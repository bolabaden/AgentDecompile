...
This file provides guidance for Claude Code when working with the AgentDecompile headless infrastructure in the `agentdecompile.headless` package.

## Quick Reference

| Item | Value |
|------|-------|
| **Main Class** | `AgentDecompileHeadlessLauncher` |
| **Default Port** | 8080 |
...
The `agentdecompile.headless` package enables AgentDecompile MCP server operation in non-GUI environments:
...
| **Claude CLI (stdio)** | Direct integration via `mcp-agentdecompile` | Ephemeral temp |
...
|                 |                  |                     |
| AgentDecompileApp Plugin  | AgentDecompileHeadless     |  mcp-agentdecompile CLI       |
...
// 2. With config file
AgentDecompileHeadlessLauncher(File configFile)
AgentDecompileHeadlessLauncher(String configFilePath)  // PyGhidra convenience

// 3. With random port
AgentDecompileHeadlessLauncher(File configFile, boolean useRandomPort)

// 4. Full control (no project)
AgentDecompileHeadlessLauncher(File configFile, boolean autoInitGhidra, boolean useRandomPort)
...
```python
import pyghidra
pyghidra.start()  # Initialize Ghidra BEFORE importing AgentDecompile
...
launcher = AgentDecompileHeadlessLauncher(None, True)  # useRandomPort=True
launcher.start()
...
**agentdecompile.properties:**
```properties
# Server settings
agentdecompile.server.options.server.port=9090
agentdecompile.server.options.server.host=127.0.0.1
agentdecompile.server.options.server.enabled=true

# Security
agentdecompile.server.options.api.key.authentication.enabled=false
agentdecompile.server.options.api.key=

# Performance
agentdecompile.server.options.debug.mode=false
agentdecompile.server.options.max.decompiler.search.functions=1000
agentdecompile.server.options.decompiler.timeout.seconds=10
agentdecompile.server.options.import.analysis.timeout.seconds=600
```
...
config = File("agentdecompile.properties")
launcher = AgentDecompileHeadlessLauncher(config)
launcher.start()
```
...
java -cp ghidra.jar:agentdecompile.jar agentdecompile.headless.AgentDecompileHeadlessLauncher

# With config file
java -cp ghidra.jar:agentdecompile.jar agentdecompile.headless.AgentDecompileHeadlessLauncher agentdecompile.properties
```
...
| **CLI (stdio)** | Ephemeral temp directory | Auto-cleanup on exit | `mcp-agentdecompile` sessions |

## Python CLI Integration (mcp-agentdecompile)
...
|---------|-------|----------|
| `IllegalStateException: Ghidra application is not initialized` | PyGhidra not started first | Initialize PyGhidra BEFORE importing AgentDecompile classes |
...
# FIX: Initialize PyGhidra BEFORE importing AgentDecompile
import pyghidra
pyghidra.start()
from agentdecompile.headless import AgentDecompileHeadlessLauncher  # Now safe
```
...
| Memory usage | ~250-400 MB (base Ghidra + AgentDecompile) |
...
**Base class:** `AgentDecompileHeadlessIntegrationTestBase.java`

```java
public abstract class AgentDecompileHeadlessIntegrationTestBase
...
|----------|-------------|
| `/src/main/java/agentdecompile/server/CLAUDE.md` | McpServerManager, transport, threading |
| `/src/main/java/agentdecompile/plugin/CLAUDE.md` | ConfigManager backends, plugin lifecycle |
| `/src/agentdecompile_cli/` | Python CLI implementation (launcher.py, stdio_bridge.py) |
