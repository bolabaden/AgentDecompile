...
import agentdecompile.plugin.config.ConfigurationBackend;
import agentdecompile.plugin.config.ConfigurationBackendListener;
import agentdecompile.plugin.config.FileBackend;
import agentdecompile.plugin.config.InMemoryBackend;
import agentdecompile.plugin.config.ToolOptionsBackend;
...
public class ConfigManager implements ConfigurationBackendListener {
    ...
    private void registerOptionsWithGhidra(ToolOptionsBackend toolBackend) {
        ...
        toolOptions.registerOption(SERVER_HOST, DEFAULT_HOST, help,
            "Host interface for the AgentDecompile MCP server (127.0.0.1 for localhost only, 0.0.0.0 for all interfaces)");
        toolOptions.registerOption(SERVER_ENABLED, DEFAULT_SERVER_ENABLED, help,
            "Whether the AgentDecompile MCP server is enabled");
        ...
        toolOptions.registerOption(REQUEST_LOGGING_ENABLED, DEFAULT_REQUEST_LOGGING_ENABLED, help,
            "Enable detailed logging of MCP tool requests and responses to agentdecompile-tools.log");
    ...
    protected void loadOptions() {
        ...
        // Get the API key - check environment variable first, then backend, then generate
        String envApiKey = System.getenv("AGENT_DECOMPILE_API_KEY");
        String apiKey;
    ...
    /**
     * Generate a default API key with AgentDecompile-UUID format
     * @return A new API key in the format "AgentDecompile-{uuid}"
     */
    private String generateDefaultApiKey() {
        return "AgentDecompile-" + UUID.randomUUID().toString();
    }
