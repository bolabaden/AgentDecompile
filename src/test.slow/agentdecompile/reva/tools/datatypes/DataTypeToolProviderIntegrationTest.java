 ...
 */
public class DataTypeToolProviderIntegrationTest extends AgentDecompileIntegrationTestBase {
    ...
    public void testGetDataTypeArchivesWithProgramPath() throws Exception {
        withMcpClient(createMcpTransport(), client -> {
            try {
                ...
                assertFalse("Tool should not fail with 'AgentDecompilePlugin is not available'",
                           resultText.contains("AgentDecompilePlugin is not available"));
        ...
        withMcpClient(createMcpTransport(), client -> {
            try {
                ...
                for (String dataType : basicTypes) {
                    ...
                    // Should not fail with "AgentDecompilePlugin is not available"
                    assertFalse("Should not fail with 'AgentDecompilePlugin is not available' for type: " + dataType,
                               resultText.contains("AgentDecompilePlugin is not available"));
                }
            } catch (Exception e) {
                fail("Test failed with exception: " + e.getMessage());
