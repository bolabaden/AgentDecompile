...
/**
 * Base class for AgentDecompile integration tests that provides common test setup
 ...
 */
public abstract class AgentDecompileIntegrationTestBase extends AbstractGhidraHeadedIntegrationTest {

    /**
     * Override this to skip plugin loading for tests that don't need GUI components.
     * @return true if the AgentDecompile plugin should be loaded, false otherwise
     */
    ...
    private static AgentDecompilePlugin sharedPlugin;
    ...
    protected AgentDecompilePlugin plugin;
    ...
    private static void setupSharedMcpServices() {
        ...
        McpServerManager existingServerManager = agentdecompile.util.AgentDecompileInternalServiceRegistry.getService(McpServerManager.class); 
        ConfigManager existingConfigManager = agentdecompile.util.AgentDecompileInternalServiceRegistry.getService(ConfigManager.class); 
        ...
        if (existingServerManager != null) {
            agentdecompile.util.AgentDecompileInternalServiceRegistry.unregisterService(McpServerManager.class); 
            agentdecompile.util.AgentDecompileInternalServiceRegistry.unregisterService(agentdecompile.services.AgentDecompileMcpService.class); 
        }
        if (existingConfigManager != null) {
            agentdecompile.util.AgentDecompileInternalServiceRegistry.unregisterService(ConfigManager.class); 
        }

        // Create and register shared ConfigManager
        sharedConfigManager = new ConfigManager(sharedTool);
        agentdecompile.util.AgentDecompileInternalServiceRegistry.registerService(ConfigManager.class, sharedConfigManager); 

        // Create and register shared McpServerManager
        sharedServerManager = new McpServerManager(sharedTool);
        agentdecompile.util.AgentDecompileInternalServiceRegistry.registerService(McpServerManager.class, sharedServerManager); 
        agentdecompile.util.AgentDecompileInternalServiceRegistry.registerService(agentdecompile.services.AgentDecompileMcpService.class, sharedServerManager); 
    ...
    public static void tearDownSharedTestEnvironment() throws Exception {
        if (sharedEnv != null) {
            ...
            agentdecompile.util.AgentDecompileInternalServiceRegistry.clearAllServices(); 
    ...
    @Before
    public void setUpAgentDecompilePlugin() throws Exception {
        // Initialize shared environment lazily on first test to ensure Ghidra system is ready
        if (sharedEnv == null) {
            synchronized (AgentDecompileIntegrationTestBase.class) {
                if (sharedEnv == null) {
                    ...
                    // Add the AgentDecompile plugin to the shared tool (if needed)
                    if (shouldLoadPlugin()) {
                        sharedTool.addPlugin(AgentDecompilePlugin.class.getName());

                        // Get the shared plugin instance
                        for (ghidra.framework.plugintool.Plugin p : sharedTool.getManagedPlugins()) {
                            if (p instanceof AgentDecompilePlugin) {
                                sharedPlugin = (AgentDecompilePlugin) p;
                                break;
                            }
                        }

                        if (sharedPlugin == null) {
                            throw new RuntimeException("Failed to load AgentDecompilePlugin in shared environment");
                        }
                    }
    ...
    @After
    public void tearDownAgentDecompilePlugin() throws Exception {
        onGhidraClose();
