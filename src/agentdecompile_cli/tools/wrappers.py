...
class GhidraTools:
    ...
    def find_function(
        ...
        # Name-based search
        functions: list[GhidraFunction] = self.get_all_functions(include_externals=include_externals)
        exact_matches: list[GhidraFunction] = [f for f in functions if name_or_address.lower() == f.getSymbol().getName(True).lower()]

        if len(exact_matches) == 1:
            return exact_matches[0]
        if len(exact_matches) > 1:
            suggestions = [f"{f.getSymbol().getName(True)}({f.getSignature()}) @ {f.getEntryPoint()}" for f in exact_matches]
            raise ValueError(f"Ambiguous match for '{name_or_address}'. Did you mean one of these: " + ", ".join(suggestions))

        # No exact matches → suggest partials
        partial_matches: list[GhidraFunction] = [f for f in functions if name_or_address.lower() in f.getSymbol().getName(True).lower()]
        if partial_matches:
            suggestions = [f"{f.getSymbol().getName(True)} @ {f.getEntryPoint()}" for f in partial_matches]
            raise ValueError(f"Function '{name_or_address}' not found. Did you mean one of these: " + ", ".join(suggestions))
    ...
    @handle_exceptions
    def find_symbols(self, name_or_address: str) -> list[GhidraSymbol]:
    ...
    @handle_exceptions
    def find_symbol(self, name_or_address: str) -> GhidraSymbol:
    ...
    def search_symbols_by_name(
        ...
        symbols: list[GhidraSymbol] = self.find_symbols(query)
        ...
        for symbol in symbols:
            if query.lower() in symbol.getName(True).lower():
                ref_count: int = len(list(rm.getReferencesTo(symbol.getAddress())))
                symbols_info.append(
                    SymbolInfo(
                        name=symbol.name,
    ...
    def _search_code_literal(
        self,
        literal_results: dict[str, Any] | None = None,
        limit: int = 10,
        offset: int = 0,
        ...
        if literal_results is not None and "documents" in literal_results:
            ...
            docs: list[str] = literal_results["documents"] or []
            metadatas: list[dict[str, Any]] = literal_results["metadatas"] or []

            # Paginate
            start_idx: int = offset
            end_idx: int = offset + limit
            paginated_docs: list[str] = docs[start_idx:end_idx]
            paginated_meta: list[dict[str, Any]] = metadatas[start_idx:end_idx] if metadatas else []
    ...
    def gen_callgraph(
        ...
        # Create unified tool instance
        callgraph_tool = CallGraphTool(self.program_info)

        try:
            # Use unified tool's MCP interface method
            result = callgraph_tool.generate_for_mcp(
                function_name_or_address=function_name_or_address,
                direction=cg_direction.value.lower(),
                display_type=cg_display_type.value.lower(),
            ...
            # Fallback to legacy implementation if unified tool fails
            logger.warning(f"Unified callgraph tool failed: {e.__class__.__name__}: {e}, falling back to legacy implementation")
            return self._gen_callgraph_legacy(
    ...
    def _gen_callgraph_legacy(
        ...
        if not selected_graph_content:
            raise ValueError(f"Cg display type {cg_display_type.value} not found for function {cg_func}.")
