...
def check_tools() -> None:
    ...
    if shutil.which("semgrep") is None:
        raise RuntimeError("semgrep not installed. Install via `pip install semgrep` or `pip install 'ghidrecomp[sast]'`")
...
def run_semgrep_scan(decomp_dir: Path, rules_dirs: list[str], out_sarif: Path) -> str:
    ...
    sarif_path, success = run_single_semgrep(configs=configs, repo_path=decomp_dir, out_dir=out_dir, timeout=600)
...
def generate_sast_summary(
    ...
    summary = {
        ...
        "total_findings": 0,
        "findings_by_severity": {"error": 0, "warning": 0, "note": 0, "none": 0},
        "findings_by_rule": {},
        "top_findings": [],
        "tools_used": [],
...
def merge_sarif_files(output_path: str, input_paths: Sequence[str]) -> None:
...
def add_sast_args_to_parser(parser: argparse.ArgumentParser) -> None:
