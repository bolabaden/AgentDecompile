...
_PROMPTS: list[dict[str, Any]] = [
    ...
    # 7. Bridge Builder – Cross-Binary Parity
    {
        ...
        "description": (
            ...
            "different platform builds). Correlates functions by name/signature and "
            "propagates names, comments, and types via rename-function, set-function-prototype, "
            "manage-comments, manage-bookmarks, and manage-function-tags (cross-binary match-function not yet implemented)."
        ),
        "arguments": [
        ...
        "messages": [
            {
                ...
                "text": (
                    ...
                    "2. Cross-binary function matching via `match-function` with a target program "
                    "path is not yet implemented. Work around this by: open or ensure the target "
                    "program is in the session, then use `list-functions` or `search-symbols` on "
                    "both source and target (switching `programPath` as needed) to correlate "
                    "functions by name or signature; use `rename-function`, `set-function-prototype`, "
                    "`manage-comments`, `manage-bookmarks`, and `manage-function-tags` on the "
                    "target program to propagate names, comments, and tags for matched functions.\n"
                    "3. Focus on the core routines already analysed in the source binary.\n\n"
                    "## Approach\n"
                    "- List functions in the source binary related to the subsystem (`list-functions` "
                    "or `search-symbols` with `programPath` set to source).\n"
                    "- For the target binary, use `list-functions` or `search-symbols` with "
                    "`programPath` set to target; correlate by name/signature, then apply "
                    "annotations on the target via `rename-function`, `set-function-prototype`, "
                    "`manage-comments`, `manage-bookmarks`, `manage-function-tags`.\n"
